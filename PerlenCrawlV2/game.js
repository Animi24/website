...
const snakes= [new Enemy(4,4,2,0,0,true)];
...
if (item==1 || item==4){
	...
	PS.glyphFade(chestX,chestY,60, {onEnd: () => PS.makeFloor(chestX,chestY)});
	PS.glyphColor(chestX,chestY,floorColor);
}
...
PS.updateEnemies = function(){
	for (var i=0; i<snakes.length; i++){
		snakes[i].setMaxHp(snakeMaxHp + Math.floor(floorsCleared/2));
		snakes[i].setCurrHp(snakes[i].getMaxHp());
		snakes[i].setDmg(snakeDMG + Math.floor(floorsCleared/3));
		snakes[i].setAlive(true);
	}
}

PS.spawnSnakes = function(){
	for (var i=0; i<snakes.length; i++) {
		let invalid = true;
		while (invalid) {
			let x = PS.random(10) - 1;
			let y = PS.random(10) - 1;
			if (PS.data(x,y) === 0){
				PS.makeSnake(x,y,i);
				invalid = false;
			}
...
PS.moveEnemy =function(){
	for (var i = 0; i < snakes.length; i++) {
		if (snakes[i].getAlive()) {
			...
			var x = snakes[i].getX();
			if (y !== player.getY()) {
				if (y > player.getY() && y > 0 && PS.data(x, y - 1) !== 1 && !moved && PS.data(x, y - 1) !== chestData && PS.data(x, y - 1) !== stairData && PS.data(x, y - 1) !== snakeData) {
					if (player.getY() === y - 1 && player.getX() === x) {
						player.setCurrHp(player.getCurrHp() - snakes[i].getDmg());
					...
					PS.makeSnake(x, y - 1, i);
					moved = true;
				}
				if (y < player.getY() && y < 9 && PS.data(x, y + 1) !== 1 && !moved && PS.data(x, y + 1) !== chestData && PS.data(x, y + 1) !== stairData && PS.data(x, y + 1) !== snakeData) {
					if (player.getY() === y + 1 && player.getX() === x) {
						player.setCurrHp(player.getCurrHp() - snakes[i].getDmg());
						PS.audioPlay("fx_bloink");
						if (player.getCurrHp() <= 0) {
			...
			if (x !== player.getX()) {
				if (x > player.getX() && x > 0 && PS.data(x - 1, y) !== 1 && !moved && PS.data(x - 1, y) !== chestData && PS.data(x - 1, y) !== stairData && PS.data(x - 1, y) !== snakeData) {
					...
					PS.makeSnake(x - 1, y, i);
					moved = true;
				}
				if (x < player.getX() && x < 9 && PS.data(x + 1, y) !== 1 && !moved && PS.data(x + 1, y) !== chestData && PS.data(x + 1, y) !== stairData && PS.data(x + 1, y) !== snakeData) {
					if (player.getX() === x + 1 && player.getY() === y) {
						player.setCurrHp(player.getCurrHp() - snakes[i].getDmg());
						PS.audioPlay("fx_bloink");
						if (player.getCurrHp() <= 0) {
...
//determines if the given coordinates is a snake returns -1 if it isnt. returns the index of the snake if it is
PS.getSnake = function(x,y){
	for (var i = 0; i < snakes.length; i++) {
		if(snakes[i].getX() === x && snakes[i].getY() === y){
			return i;
		}
	}
	return -1;
...
PS.keyDown = function( key, shift, ctrl, options ) {
	...
	if ((key == PS.KEY_ARROW_UP || key == 119) && player.getAlive()){
		if (player.getY() > 0 && PS.data(player.getX(),player.getY()-1) !== 1){
			var snake = PS.getSnake(player.getX(), player.getY()-1);
			if (snake !== -1 && snakes[snake].getAlive()){
				snakes[snake].setCurrHp(snakes[snake].getCurrHp() - player.getDmg());
				PS.audioPlay("fx_swoosh");
				if (snakes[snake].getCurrHp() <= 0){
					snakes[snake].setAlive(false);
					PS.audioPlay("fx_rip");
					PS.makeFloor(snakes[snake].getX(), snakes[snake].getY());
					PS.glyph(snakes[snake].getX(),snakes[snake].getY(), 0);
				}
			}
	...
	else if ((key == PS.KEY_ARROW_DOWN || key == 115) && player.getAlive()){
	...
	else if ((key == PS.KEY_ARROW_RIGHT || key == 100) && player.getAlive()){
		if (player.getX() < 9 && PS.data(player.getX()+1,player.getY()) !== 1){
			var snake = PS.getSnake(player.getX()+1, player.getY());
			if (snake !== -1 && snakes[snake].getAlive()) {
				snakes[snake].setCurrHp(snakes[snake].getCurrHp() - player.getDmg());
				PS.audioPlay("fx_swoosh");
				if (snakes[snake].getCurrHp() <= 0){
	...
	else if ((key == PS.KEY_ARROW_LEFT || key == 97) && player.getAlive()){
